...
DCL-PROC Actual_Param_Init;
    ...
    self.left = var_node;
    self.token.type = TokenTypeID('PARAMCALL');
    self.right = type_node;
...
DCL-PROC Parser_Statement;
    ...
    ELSEIF self.current_token.type = TokenTypeID('ID')
        ...
        RETURN parser_proccall_statement(self);
    ELSEIF self.current_token.type = TokenTypeID('ID');
...
DCL-PROC Parser_Proccall_Statement;
    DCL-PI *N POINTER;
        self LIKEDS(Parser_t);
    END-PI;
    ...
    Actual_Params = *NULL;
    IF Self.Current_Token.Type <> TokenTypeID('RPAREN');
    ...
    //Node = Parser_ProcedureCall(Proc_Name:Actual_Params:token);
