...
DCL-PROC Interpreter_Factor;
    ...
    ELSEIF atoken.type = LPAREN;
        Interpreter_Eat(self:LPAREN);
        result = Interpreter_expr(self);
        Interpreter_Eat(self:RPAREN);
        RETURN result;
...
DCL-PROC Interpreter_Expr;
    ...
    result = %INT(Interpreter_Term(self));
    ...
    DOW self.current_token.type = PLUS
        ...
        IF aToken.type = PLUS;
            ...
            Result += %INT(Interpreter_Term(self));
        ELSEIF aToken.type = MINUS;
            Interpreter_Eat(self:MINUS);
            Result -= %INT(Interpreter_Term(self));
        ENDIF;
